...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     * 
     * 而且： 为了避免resize和转化为红黑树之间进行冲突，还应该设置一个长度的阈值，至少是4*阈值，这里采取了64
     * 用来权衡 调整大小的资源成本和转化红黑树的成本
     * 
     * 当链表长度小于等于6时再转化为链表，因为长度为6即使O(n)也没有多少复杂度
     * 
     * 跳开了长度为7的情况是避免 HashMap在频繁插入删除的情况下 链表长度在8左右频繁变化导致 链表和红黑树的频繁变更
     * 
     * 这种阈值的设定是根据统计结果做出的 一种优化的权衡，只能说在统计结果下效果比较好
     *
     ...
     * 
     * HashMap可以存储 key =null 和value==null 
     */
    ...
    /**
     * modCount是让集合视图上的迭代器快速失败的实现
     * 就是在进行forEach遍历后 会检查modCount是否与遍历前modcount是否一样，不一样的抛出ConcurrentModificationException
     *  所以forEach里不能执行元素的修改、删除操作-> 删除应该使用iterator的方式
     * 
    ...
    final Node<K,V> getNode(Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if (first.hash == hash && // always check first node first指向 hash(key)%n 的位置，如果这个位置hash与key的hash相等则直接返回value
                ((k = first.key) == key || (key != null && key.equals(k))))
                return first;
            if ((e = first.next) != null) {// 如果有成链 则判断first结点是数节点 还是链表节点；
                if (first instanceof TreeNode)// 树节点 则根据hash和key去树里查询
                    return ((TreeNode<K,V>)first).getTreeNode(hash, key);
                do {
                    if (e.hash == hash &&  // 如果是链表则遍历一遍查询
